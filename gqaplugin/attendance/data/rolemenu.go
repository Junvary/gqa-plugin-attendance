...
func (s *sysRoleMenu) LoadData() error {
	return gqaGlobal.GqaDb.Table("sys_role_menu").Transaction(func(tx *gorm.DB) error {
		...
		if count != 0 {
			...
			gqaGlobal.GqaLogger.Warn("[GQA-Plugin] --> sys_role_menu 表中attendance插件数据已存在，跳过初始化数据！", zap.Any("数据量", count))
			return nil
		}
		...
		gqaGlobal.GqaLogger.Info("[GQA-Plugin] --> attendance插件初始数据进入 sys_role_menu 表成功！")
		return nil
	})
}
